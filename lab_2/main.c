...
{
	Transform* transform = pDFAState -> firstTran;
	while(transform != NULL){
		if (transform -> TransformChar == TransformChar ){
			return transform;
		}
...
void Closure(NFAState* State, NFAState** StateArray, int* Count)
{
	...
	// 利用栈进行非递归的先序遍历
	while(State != NULL || !NFAStateStackEmpty(&StateStack)){
		while( State != NULL ){
		...
		if (!NFAStateStackEmpty(&StateStack)){
